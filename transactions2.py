...
class FraudSimulator:
    ...
    def generate_transaction(self, sender_id, receiver_id, current_time, fraud_ratio=0.005):
        ...
        if merchant_category in self.fraud_merchants and random.random() < 0.012:
            is_fraud = True
        ...
        if receiver_id in self.mule_accounts and random.random() < 0.025:
            is_fraud = True
        ...
        amount_to_avg_ratio = round(amount / (sender_avg_txn + 1e-6), 2)
        device_match = int(device_fingerprint == sender_device_fingerprint)

        # === Account balance fraud check ===
        if amount > sender_balance and random.random() < 0.002:
            is_fraud = True
